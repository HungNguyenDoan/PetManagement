...
import com.project.petmanagement.petmanagement.models.entity.QNutritiousFood;
...
public class NutritiousFoodService {
    ...
    // public List<NutritiousFood> searchNutritiousFoodByFilter(String keyword, Long
    ...
    // return
    // nutritiousFoodRepository.findByNameContainingAndFoodTypeAndSpecies(keyword,
    // existingFoodType,
    // existingSpecies);
    // } else {
    ...
    // .orElseThrow(() -> new DataNotFoundException("Can not find species with ID: "
    // + speciesId));
    // return nutritiousFoodRepository.findByNameContainingAndSpecies(keyword,
    // existingSpecies);
    ...
    // return nutritiousFoodRepository.findByNameContainingAndFoodType(keyword,
    // existingFoodType);
    ...
    // } else {
    // throw new IllegalArgumentException("At least one parameter must be
    // provided.");
    // }
    // }
    // }
    public List<NutritiousFood> searchNutritiousFoodByFilter(String keyword, Long speciesId, Long foodTypeId) {
        QNutritiousFood nutritiousFood = QNutritiousFood.nutritiousFood;
        BooleanExpression predicate = nutritiousFood.isNotNull();
        if (foodTypeId != null) {
            predicate = predicate.and(nutritiousFood.foodType
                    .eq(foodTypeRepository.findById(foodTypeId).get()));
        }
        if (speciesId != null) {
            predicate = predicate.and(nutritiousFood.species.eq(speciesRepository.findById(speciesId).get()));
        }
        if (keyword != null && !keyword.isEmpty()) {
            StringExpression nutritiousName = nutritiousFood.name;
            predicate = predicate.and(nutritiousName.like("%" + keyword + "%"));
        }
        return (List<NutritiousFood>) nutritiousFoodRepository.findAll(predicate);
    }
}
